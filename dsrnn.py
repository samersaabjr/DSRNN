...
"""Abstract object representing a DSRNN cell, called DSRNNCell.
Every `DSRNNCell` must have the properties below and implement `call` with
...
#    current_state = special_math_ops.einsum('ijk,jk->ik', state, self._kernel_A) + special_math_ops.einsum('ij,j->ij', current_state, (1-math_ops.reduce_sum(self._kernel_A, 0)))
  ...
  def get_config(self):
    ...
    base_config = super(DSRNNCell, self).get_config()
